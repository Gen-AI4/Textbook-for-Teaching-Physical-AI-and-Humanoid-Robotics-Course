...
// Create a new chat
app.post("/", authGuard, zValidator("json", z.object({
...
// Get chat messages
app.get("/:chatId/messages", authGuard, zValidator("param", z.object({
...
app.post("/:chatId/message", authGuard, zValidator("param", z.object({
    chatId: z.string().uuid(),
    ...
    try {
        ...
        const relevantChunks = await db
            .select()
            .from(chunks)
            .where(and(eq(chunks.userId, user.id), sql `content LIKE '%${content}%'` // Simple text search for demo
        ))
            .limit(5);
